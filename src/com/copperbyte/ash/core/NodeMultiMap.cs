...
namespace com.copperbyte.ash.core
{
	...
	internal interface INodeMultiMap
	{
		void Add(Node node);
	...
	public class NodeMultiMap<CK, NT> : INodeMultiMap, IEnumerable<KeyValuePair<CK, List<NT> > > where CK:Component where NT:Node
	{
		...
		public NodeMultiMap()
		{
			mNodes = new Dictionary<CK, List<NT> >();
		...
		{
			CK key = (CK)node.entity.Get(typeof(CK));
			if(!mNodes.ContainsKey(key))
				mNodes.Add(key, new List<NT>());

			List<NT> nodeList = mNodes[key];

			if(nodeList.Contains((NT)node))
				return;

			nodeList.Add((NT)node);
		...
		public void Remove( Node node )
		{
			...
			List<NT> nodeList = mNodes[(CK)node.entity.Get(typeof(CK))];

			if(!nodeList.Contains((NT)node))
				return;

			nodeList.Remove((NT)node);
		...
		public void RemoveAll()
		{
			foreach(CK key in mNodes.Keys) {
				List<NT> list = mNodes[key];
				foreach(NT node in list) {
					list.Remove(node);
					if(nodeRemoved != null)
						nodeRemoved(node);
				}
		...
		public Dictionary<CK, List<NT> >  Get() 
		{
			return mNodes;
		}
